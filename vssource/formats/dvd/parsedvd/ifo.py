...
class IFO0(SectorReadHelper):
    def _load(self) -> None:
        ...
        tt_srpt = []
        for a in range(num):
            type,nr_of_angles,nr_of_ptts,parental,title_set_nr,vts_ttn,title_set_sector = self._unpack_byte([1, 1, 2, 2, 1, 1, 4])
            entry = dict()
...
class IFOX(SectorReadHelper):
    ...
    def vts_pgci(self):
        ...
        for _ in range(nr_pgcs):
            ...
            #print(json.dumps(cell_playback,sort_keys=True, indent=4))
            #sys.exit(0)
            pgcs += [{
    ...
    def vts_ptt_srpt(self):
        ...
        num,res,end = self._unpack_byte([2,2,4])
    ...
    def vts_c_adt(self):
        ...
        cnt = (end + 1 - 6) // 12
        for _ in range(cnt):
            vob_id,cell_id,res,start_sector,last_sector = self._unpack_byte([2,1,1,4,4])
            vts_c_adt += [{
                "vob_id": vob_id,
                "cell_id": cell_id,
    ...
    def vtsi_mat(self):
        ...
        #vts_video_attr["picture_size"] = (vb0 & 0b00110000) >> 4
        ...
        for _ in range(num_audio):
            ...
            audio_format = (buf[0] & 0b11000000) >> 6
            if lang_type:
